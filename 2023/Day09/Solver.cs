...
internal class Solver
{
    public Solver()
    {
        Debug.Assert(PredictPreceding([10, 13, 16, 21, 30, 45]) == 5);
        Debug.Assert(Solve("""
0 3 6 9 12 15
1 3 6 10 15 21
10 13 16 21 30 45
""") == 2);
    ...
    public int Solve(string input)
    {
        ...
        var predictions = rows.Select(PredictPreceding);
    ...
    private int PredictPreceding(int[] values)
    {
        return PredictNext(values.Reverse().ToArray());
    }
    ...
    private int PredictNext(int[] values)
    {
        ...
        for (int i = sequences.Count - 1 - 1; i >= 0; i--)
        {
            ...
            // predicts[i] = sequences[i][0] - predicts[i + 1]; // if you want PredictPreceding without Reverse
